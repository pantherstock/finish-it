/**
 * Finish It (fable-5) — Cloudflare Worker.
 *
 * Serves the static app (via the `assets` config in wrangler.jsonc) and one API
 * route: POST /api/lesson — the Claude-backed article→lesson engine from the
 * product brief. The Anthropic key lives here as a Worker secret and never
 * reaches the browser (the repo's no-browser-side-LLM-keys rule).
 *
 * Raw fetch instead of the Anthropic SDK on purpose: this repo's constraint is
 * "no package.json, no toolchain", and a single-file Worker deploys as-is.
 *
 * Setup:
...
// Mirrors the front-end's lesson shape: buildLesson() in index.html returns
// {summary, chunks:[{text,kicker}], questions} — this schema is the same data
// with the question riding on each chunk (answer_index instead of answer text).
const LESSON_SCHEMA = {
  type: "object",
      ...
      items: {
        ...
        properties: {
          ...
          kicker: {
            ...
            description: "Short section label (max 6 words) on the chunk where a new section starts; null otherwise.",
          },
          question: {
            anyOf: [
              {
...
4. chunk.kicker: if the article has real sections, put a short label (max 6 words, based on the article's own heading) on the chunk where each section starts; null on every other chunk. If the article has no real sections, kicker is null everywhere.
5. chunk.question: one quick recall question answerable ONLY from that chunk. Exactly 4 short options, exactly one correct, plausible distractors, never a trick, never ambiguous. Vary the style across chunks: a key term, a number, a name, a cause or consequence. If a chunk supports no fair question, set question to null rather than forcing an awkward one.
6. summary: one sentence, under 200 characters, capturing the article's core point — written from the article, not generic.`;
...
async function lesson(request, env) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      // Sonnet 4.6 per the product brief's quality/cost call ($3/$15 per MTok ≈
      // 2–5¢ per article). Override via the MODEL var in wrangler.jsonc:
      // "claude-haiku-4-5" to cut cost ~3x, "claude-opus-4-8" for max quality.
      model: env.MODEL || "claude-fable-5",
      max_tokens: 16000,
      output_config: {
        effort: "medium",
