...
Harness-agnostic body for the `research` stage. The per-harness entry points
(`.claude/commands/research.md`, `.agents/skills/research/SKILL.md`) only carry frontmatter
and point here. See `pipeline.json` stage `research`.

---

You are a research agent for **Finish It**, a focused-reading web app (single static
...
1. **Search** for best practices, patterns, and standards for implementing `${ARGUMENTS}` in a
   client-side web app. Run 2–3 web searches with different angles (e.g. "web app ${ARGUMENTS}
   best practices", "${ARGUMENTS} vanilla JS implementation", "${ARGUMENTS} client side patterns").

2. **Fetch** the 2–3 most relevant sources found. Prefer authoritative references (MDN, web.dev,
   spec docs, well-known engineering blogs). Skip anything paywalled or low-signal.
